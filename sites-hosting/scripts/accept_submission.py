...
def accept(payload):
    ...
    if not match:raise ValueError('Submission must include the JSON proposal generated by the website')
    data=json.loads(match[1]);kind=data.get('kind')
    if kind not in ('event','feed','page'):raise ValueError('Unknown submission type')
    for k,n in [('title',180),('url',1200),('organiser',120),('description',600),('location',300),('timezone',80)]:
    ...
    if kind in ('feed','page'):
        ...
        entry={'id':'submitted-'+str(issue['number']),'name':data['title'],'url':data['url'],'adapter':'auto','topics':data['topics'],'region':'Community submitted','submission':issue['html_url']}
